...
vector<double> Texture::getBackgroundColor(tuple<double, double, double> direction)
{
    direction = lib.norm(direction);
    double x = round((atan2(get<2>(direction), get<0>(direction))/ (2 * M_PI) + 0.5) * widht);

    double y = round(acos(-get<1>(direction)) / (M_PI * widht));

    return this->backgroundPixels[y][x];
}
